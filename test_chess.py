...
class TestChess:
    ...
    def test1_get_valid_peaceful_moves(self, game):
        ...
        with patch.object(game, 'get_piece', return_value=Player.EMPTY):
            ...
            correct_moves = [(1, 3), (2, 2), (4, 2), (5, 3), (5, 5), (4, 6), (2, 6), (1, 5)]
    ...
    def test2_knight_get_valid_peaceful_moves(self, game):
        ...
        correct_value = [(2, 1), (1, 2)]
        assert set(returned_value) == set(correct_value)

    # knight in the  board frame
    def test3_knight_get_valid_peaceful_moves(self, game):
        knight = Knight('n', 3, 7, Player.PLAYER_1)

        def mock_get_piece(r, c):
            if (r, c) != (3, 7) and (0 <= r < 8) and (0 <= c < 8):
                return Player.EMPTY
            else:
                return None
    ...
    # allies and enemy
    def test3_knight_get_valid_piece_takes(self, empty_the_board):
        empty_the_board.board[2][1] = Knight('n', 2, 1, Player.PLAYER_2)
        knight = Knight('n', 0, 0, Player.PLAYER_1)
        empty_the_board.board[0][0] = knight
        moves = knight.get_valid_piece_takes(empty_the_board)
        expected_moves = [(2, 1)]
        assert set(moves) == set(expected_moves)

    def test_evaluate_board(self, empty_the_board):
        empty_the_board.board[3][4] = Knight('n', 3, 4, Player.PLAYER_1)
        empty_the_board.board[2][5] = Bishop('b', 2, 5, Player.PLAYER_2)
        chess_ai = ai_engine.chess_ai()
        score = chess_ai.evaluate_board(empty_the_board, Player.PLAYER_1)
